...
class heart_rate():
    ...
    def adjust_rr_interval(self, ind):
        if ind >= 8:
            self.RR1 = np.diff(self.peaks[ind-8:ind+1]) / self.fs
    
            self.RR_Average1 = np.mean(self.RR1)
            self.RR2 = [r for r in self.RR1 if self.RR_Low_Limit < r < self.RR_High_Limit]
    
            if len(self.RR2) > 0:
                self.RR_Average2 = np.mean(self.RR2)
            else:
    ...
    def searchback(self, peak_val, RRn, sb_win):
        if RRn > self.RR_Missed_Limit:
            start = max(0, peak_val - sb_win + 1)
            end = min(len(self.m_win), peak_val + 1)
            win_rr = self.m_win[start:end]
            ...
            if len(coord) > 0:
                ...
                start = max(0, x_max - self.win_150ms)
                end = min(len(self.b_pass), x_max + 1)
                win_rr = self.b_pass[start:end]
                coord = np.where(win_rr > self.Threshold_F1)[0]
    
                if len(coord) > 0:
                    r_max = coord[np.argmax(win_rr[coord])]
    ...
    def find_t_wave(self, peak_val, RRn, ind, prev_ind):
        if self.m_win[peak_val] >= self.Threshold_I1:
            if ind > 0 and 0.20 < RRn < 0.36:
                curr_slope = max(np.diff(self.m_win[max(0, peak_val - round(self.win_150ms / 2)):peak_val + 1]))
                last_slope = max(np.diff(self.m_win[max(0, self.peaks[prev_ind] - round(self.win_150ms / 2)):self.peaks[prev_ind] + 1]))
        ...
        elif self.Threshold_I1 > self.m_win[peak_val] > self.Threshold_I2:
            self.NPKI = 0.125 * self.m_win[peak_val] + 0.875 * self.NPKI
            self.NPKF = 0.125 * self.b_pass[ind] + 0.875 * self.NPKF
