...
for fnum in range(2,22):
    ...
    subjects = ['A','B','C','C','C','C','D','D','D','D','E','E','E','E','F','F','F','F','G','G','G','G']
    # Subject = subjects[(fnum // 4) % len(subjects)]
    Subject = subjects[fnum]
    ...
    ax.set_title(f'Boxplot: Rest vs Task (All Channels {metrics[0]},{metrics[1]})\nSubject {Subject} | {task} | {time} | {feature_name}', fontsize=16)
    ...
    # for i in range(0,1):
        ...
        # Adding titles and labels
        ax.set_title(f'Boxplot: Rest vs Task ({ch_names[i]})\nSubject {Subject} | {task} | {time} | {feature_name}', fontsize=16)
        ax.set_xticklabels(['Rest', 'Task'], fontsize=12)
        ax.set_ylabel('Value ', fontsize=12)
        # ax.set_ylim(-7e-5,7e-5)
    ...
    print("%0.2f LDA accuracy with a standard deviation of %0.2f" % (acc_scores_lda.mean(), acc_scores_lda.std()))
    
    roc_scores_lda = cross_val_score(pipe, X, y, cv=skf, scoring='roc_auc')
    print(roc_scores_lda)
    
    print("LDA ROC AUC Score: ", roc_scores_lda.mean())
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        plt.show()
        fig.savefig(f'images/ml/dataset {fnum}/dataset {fnum} fold {fold_idx}, LDA')
    ...
    print("%0.2f SVM accuracy with a standard deviation of %0.2f" % (acc_scores_svm.mean(), acc_scores_svm.std()))
    
    roc_scores_svm = cross_val_score(pipe, X, y, cv=skf, scoring='roc_auc')
    print(roc_scores_svm)
    
    print("SVM ROC AUC Score: ", roc_scores_svm.mean())
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        plt.show()
        fig.savefig(f'images/ml/dataset {fnum}/dataset {fnum} fold {fold_idx}, SVM')
...
# aa_acc_lda = acc_scores_lda.copy()
# aa_roc_lda = roc_scores_lda.copy()
# ab_acc_svm = acc_scores_svm.copy()
# ab_roc_svm = roc_scores_svm.copy()
